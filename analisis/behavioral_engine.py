...
class BehavioralEngine:
    ...
    def simular_sesion_analisis(self, duracion_segundos=180, atencion_media=70.0, nivel_hiperactividad=6.0):
        """
        Simula los datos de una sesión de análisis completa para cuando se opera sin cámara física 
        o para demostrar el sistema de inmediato con datos verosímiles.
        """
        self.reset_session()
        self.frames_totales = int(duracion_segundos * self.fps)
        
        # Calcular proporciones
        self.frames_atencion = int(self.frames_totales * (atencion_media / 100.0))
        self.frames_distraccion = self.frames_totales - self.frames_atencion
        
        # Generar timeline con ruido clínico coherente
        # El TDAH suele presentar rachas de atención seguidas de distracciones
        estado_atencion = 1
        racha_actual = 0
        
        for seg in range(1, duracion_segundos + 1):
            # Transiciones de estado basadas en Markov y perfil de atención media
            prob_distraerse = 0.08 if estado_atencion == 1 else 0.3
            if atencion_media < 50:
                prob_distraerse += 0.1
                
            rand = np.random.rand()
            if estado_atencion == 1 and rand < prob_distraerse:
                estado_atencion = 0
                racha_actual = 0
            elif estado_atencion == 0 and rand < (0.25 if atencion_media < 60 else 0.4):
                estado_atencion = 1
                racha_actual = 0
            
            racha_actual += 1
            
            # Eventos de distracción largos
            if estado_atencion == 0 and racha_actual == 2:  # Duró 2 seg distraído
                self.eventos_distraccion += 1
                
            # Generar Fidgeting oscilatorio
            # La hiperactividad fluctúa. Hacemos que suba en momentos de distracción
            base_fidgeting = nivel_hiperactividad * 0.7
            if estado_atencion == 0:
                base_fidgeting += np.random.uniform(1.0, 3.0)  # Más movimiento al distraerse
            else:
                base_fidgeting += np.random.uniform(-1.0, 1.0)
                
            fidgeting_score = max(0.0, min(10.0, base_fidgeting))
            fidgeting_score = round(fidgeting_score + np.random.normal(0, 0.4), 1)
            fidgeting_score = max(0.0, min(10.0, fidgeting_score))
            
            # Generar Yaw / Pitch coherentes
            if estado_atencion == 1:
                yaw = np.random.normal(0, 4.0)
                pitch = np.random.normal(0, 3.0)
            else:
                # Rotado a los lados (Yaw fuerte) o hacia abajo (Pitch fuerte)
                yaw = np.random.choice([np.random.normal(25.0, 5.0), np.random.normal(-25.0, 5.0)])
                pitch = np.random.normal(-20.0, 6.0)
        ...
        # Reconstruir intervalos de distracción de la simulación
        inicio_d = None
        for data in self.timeline_data:
            if data["atencion"] == 0:
                if inicio_d is None:
                    inicio_d = data["segundo"]
            else:
                if inicio_d is not None:
                    dur = data["segundo"] - inicio_d
                    if dur >= self.segundos_para_evento_distraccion:
                        self.intervalos_distraccion.append({
                            "inicio": inicio_d,
                            "fin": data["segundo"],
                            "duracion": dur
